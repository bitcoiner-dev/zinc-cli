...
## 2024-06-15 - CRITICAL Path Traversal in Snapshot & Profile Paths
**Vulnerability:** User input for snapshot names and profile names was concatenated into file paths without validation using `Path::join` (e.g., `root.join("snapshots").join(config.profile)` and `snap_dir.join(format!("{name}.json"))`).
**Learning:** `Path::join` explicitly replaces the base directory if the appended string is an absolute path, and allows escaping the base directory if `..` is used, causing critical path traversal that could overwrite arbitrary system files like `/etc/passwd`.
**Prevention:** Always validate user-provided strings used in file paths via a strict allowlist (e.g. `validate_file_name` ensuring only alphanumeric, underscores, and dashes) before passing them to `Path::join`.
