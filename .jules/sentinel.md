...
## 2024-03-24 - Path Traversal Vulnerabilities
**Vulnerability:** Path traversal vulnerabilities using `Path::join` without validating user input (e.g. `config.profile` or snapshot `name`).
...
**Prevention:** Always validate user-provided strings used in file paths (e.g., when calling `Path::join`) to ensure they only contain safe characters (using strict allowlist validation like alphanumeric, underscores, dashes) to prevent path traversal vulnerabilities.
