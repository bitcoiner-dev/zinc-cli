...
**Vulnerability:** User-provided snapshot names in `snapshot save` and `snapshot restore` were directly appended to the snapshot directory using `Path::join` without validation.
**Learning:** In Rust, `Path::join` replaces the base path entirely if the appended string is an absolute path (e.g., `/etc/passwd`). It also allows traversal with `..`.
**Prevention:** Always validate user input strings intended for file names to ensure they only contain safe characters (e.g., alphanumeric, underscores, dashes) before passing them to `Path::join`.
