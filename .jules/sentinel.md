...
## 2024-03-24 - Unsanitized User Input in PathBuf::join
**Vulnerability:** The CLI application was taking unsanitized user inputs, like profile names and snapshot names, and passing them directly into `PathBuf::join(format!("{}.json", name))`. This allowed path traversal (e.g. `../../../../etc/passwd`), because Rust's `PathBuf::join` replaces the entire path if given an absolute path, and happily traverses directories if given `..`.
**Learning:** Functions like `PathBuf::join` or `Path::join` are vulnerable to path traversal attacks when the right-hand side contains unsanitized user-supplied strings.
**Prevention:** Always explicitly validate user-provided strings used in file paths against a strict allowlist (e.g., using `crate::utils::validate_file_name`) to prevent directory traversal and absolute path injections.
