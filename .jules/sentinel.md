...
## 2024-05-24 - [CRITICAL] Fix Path Traversal in Snapshot Commands
**Vulnerability:** The snapshot command allowed arbitrary path traversal when users provided a snapshot name like `../../etc/passwd`.
**Learning:** `Path::join` allows path traversal when unsanitized input is appended, which can allow an attacker or misconfigured script to overwrite or read unintended files outside of the target directory.
**Prevention:** Always sanitize or validate user-provided paths using strict allowlists (e.g., `validate_file_name` ensuring only alphanumeric, underscore, and dash characters) before using them in file paths or `Path::join`.
