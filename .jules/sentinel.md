...
## 2024-06-06 - Path Traversal in Snapshot Commands
**Vulnerability:** User-provided snapshot names in `Save` and `Restore` commands were passed directly to `PathBuf::join` without validation, allowing directory traversal (e.g., `../etc/passwd`).
**Learning:** Rust's `Path::join` explicitly traverses upwards if given `..` and replaces the base directory if given an absolute path. It does not sanitize inputs, leading to path traversal vulnerabilities if user input is not validated.
**Prevention:** Always validate user-provided strings used in file paths against a strict allowlist (e.g., alphanumeric, dashes, underscores) before passing them to `PathBuf::join` or other path resolution functions.
