...
## 2024-05-09 - Path Traversal in Snapshot Name Configuration
**Vulnerability:** Snapshot `Save` and `Restore` commands allowed arbitrary path traversal (e.g. `../`) via the unvalidated user input parameter `name`.
**Learning:** `Path::join` explicitly resolves `..` elements and overwrites base directories when parsing absolute paths, risking writes/reads to unintended system paths. File names created from user input must be validated strictly on content.
**Prevention:** Always implement an explicit allowlist-based filename validation check (e.g., alphanumeric, underscores, and dashes only) on all user-supplied filenames before constructing file paths with `Path::join`.
