...
**Vulnerability:** The snapshot command allowed user-supplied snapshot names directly into `Path::join`, enabling path traversal (e.g., passing `../etc/passwd` to read or overwrite sensitive files outside the snapshot directory).
**Learning:** `Path::join` appends strings and will process `..` to traverse upward, or completely replace the base path if the input looks like an absolute path. Any user input passed to file path construction must be strictly validated.
**Prevention:** Always use strict allowlist validation (like `validate_file_name` which only allows alphanumeric, dashes, and underscores) on user-provided strings before using them to construct paths.
