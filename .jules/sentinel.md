...
## 2025-02-14 - Fix Path Traversal in Snapshot Command
**Vulnerability:** A path traversal vulnerability existed in `src/commands/snapshot.rs` where user-provided snapshot names in `SnapshotAction::Save` and `SnapshotAction::Restore` were directly appended to directory paths using `Path::join` without validation.
**Learning:** `Path::join` allows directory traversal using `..` or absolute paths, creating a critical vulnerability if unsanitized user input is accepted, allowing malicious reads or writes outside the intended snapshot directory.
**Prevention:** Always validate user-provided filenames against strict character allowlists (e.g., alphanumeric, underscores, and dashes) before passing them to file system functions like `Path::join`.
