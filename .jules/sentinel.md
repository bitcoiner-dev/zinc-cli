...
## 2024-06-14 - Path Traversal Prevention in Profiles and Snapshots
**Vulnerability:** The CLI application allowed unsanitized user input (`config.profile` and snapshot `name`) to be appended to file paths via `Path::join` without validation. If a user provided a profile name like `../../../etc/passwd`, it could lead to critical path traversal vulnerabilities, allowing files outside the intended directories to be read or written.
**Learning:** `Path::join` in Rust explicitly traverses upwards if given `..` and replaces the base directory if given an absolute path (e.g., `/etc/passwd`). Relying on raw string inputs for file paths is insecure and can lead to arbitrary file access.
**Prevention:** Always validate user-provided strings used in file paths to ensure they only contain safe characters. Use a strict allowlist validation function (like `crate::utils::validate_file_name`) that restricts characters to alphanumeric, underscores, and dashes before passing them to `join` or any file-system operation.
