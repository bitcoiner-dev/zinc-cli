...
## 2024-05-08 - Path Traversal Vulnerability in Snapshot Names
**Vulnerability:** The snapshot command allowed arbitrary strings as snapshot names (`name` argument), which were directly formatted into a file path (`snap_dir.join(format!("{name}.json"))`). This allowed a path traversal vulnerability where an attacker could provide names like `../sensitive_file` to read or overwrite arbitrary JSON files outside the snapshot directory.
**Learning:** Functions that accept user input and use it to construct file paths must strictly validate the input, rather than just relying on `Path::join`, as `Path::join` traverses directories when encountering `..` or absolute path strings.
**Prevention:** Always validate user-provided strings used in file paths against a strict character allowlist (e.g., alphanumeric, underscores, and dashes) using functions like `crate::utils::validate_file_name` to prevent path traversal vulnerabilities.
