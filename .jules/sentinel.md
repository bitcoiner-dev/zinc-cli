...
## 2024-07-13 - Path Traversal via Config Profile Name
**Vulnerability:** Path traversal was possible via the `config.profile` string. `Path::join` behaves dangerously in Rust when it receives an absolute path or relative directory traversals like `..`, which allowed local config files to manipulate file writing locations.
**Learning:** Functions that implicitly construct paths based on seemingly innocuous local configuration attributes (like a profile name) must still validate that those strings are safe filenames. Furthermore, duplicate directory creations using `std::fs::create_dir_all` bypass secure permission defaults set in helper functions.
**Prevention:** Always validate user-provided strings used in file path components (e.g. `crate::utils::validate_file_name`) before calling `.join()`. Also, avoid duplicating side-effect logic (like directory creation) that is already safely handled by lower-level path resolution functions.
