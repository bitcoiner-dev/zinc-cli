    ...
    for c in name.chars() {
        if !c.is_ascii_alphanumeric() && c != '_' && c != '-' {
            return Err(AppError::Invalid(format!(
                "invalid character in filename: '{}'. Only alphanumeric characters, underscores, and dashes are allowed.",
                c
            )));
        }
    }
...
mod tests {
    ...
    fn test_validate_file_name() {
        ...
        assert!(validate_file_name("Another-Valid_Name").is_ok());
        assert!(validate_file_name("12345").is_ok());
        ...
        assert!(matches!(
            validate_file_name("invalid/name"),
            Err(AppError::Invalid(_))
        ));
        assert!(matches!(
