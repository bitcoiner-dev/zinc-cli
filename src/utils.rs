...
pub fn validate_file_name(name: &str) -> Result<(), crate::error::AppError> {
    if name.is_empty() {
        return Err(crate::error::AppError::Invalid("file name cannot be empty".to_string()));
    }
    if !name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-') {
        return Err(crate::error::AppError::Invalid(format!("invalid characters in file name: {name}")));
    }
    Ok(())
}
