...
pub fn validate_file_name(name: &str) -> Result<(), AppError> {
    if name.is_empty() {
        return Err(AppError::Invalid("invalid file name: cannot be empty".to_string()));
    }
    for c in name.chars() {
        if !c.is_ascii_alphanumeric() && c != '_' && c != '-' {
            return Err(AppError::Invalid(
                "invalid file name: must only contain alphanumeric characters, underscores, and dashes".to_string(),
            ));
        }
    }
    Ok(())
